...
class GetDashboard(BaseView):
    ...
    all_graph = {
        'DAU': {'name_graph': 'DAU and DAU new users',
        ...
        'post_count': {'name_graph': 'Count posts and count post from new users',
        ...
        'votes_comments': {'name_graph': 'Count comments and count votes',
        ...
        'usd': {'name_graph': 'Count payouts (STEEM * today\'s USD rate)',
        ...
        'steem': {'name_graph': 'Count payouts (in STEEM)',
                  'name_data_line_1': 'fee users',
                  'name_data_line_2': 'fee steepshot',
                  'name_div': 'graph5',
                  'urls': [
    ...
    #     return res_graph
    def _get_data_graph(self, name_graph, platform, api_query=None):
        if not api_query:
            api_query = self._make_api_query(self.date_to, self.date_from)
        graph = self.all_graph.get(name_graph)
        if name_graph == 'steem':
            api_query['currency'] = 'steem'
        print(api_query, 'API!')
        try:
        ...
        if platform == 'steem':
            platform = settings.STEEM_V1
        elif platform == 'golos':
            platform = settings.GOLOS_V1
        for i in graph['urls']:
            try:
                data = requests.get(settings.REQUESTS_URL.get(i['url'], '{url}').format(url=platform), params=api_query).json()
                if 'result' in data:
                    data = data['result']
            except JSONDecodeError as e:
                logger.error('Failed to parse json: {err}.'.format(err=e))
            ...
            if name_graph == 'LTV':
                for key in graph['list_keys_ltv']:
                    res_graph['data_y'].append([d[key] for d in self._sort_data_from_request(data, i['data_x'], 'ltv')])
            else:
                res_graph['data_y'].append(self._sort_data_from_request(data, i['data_x'], i['data_y']))
        else:
            res_graph['data_x'].append(self._sort_data_from_request(data, i['data_x'], i['data_y'], last_iter=True))
        return res_graph
    ...
    def get(self, request):
        if request.GET.get('graph'):
            ...
            if request.GET.get('date_from') or request.GET.get('date_to'):
                ...
                data = self._get_data_graph(name_graph, platform, api_query=api_query)
                return JsonResponse(data)
            data = self._get_data_graph(name_graph, platform)
            return JsonResponse(data)
