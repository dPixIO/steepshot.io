...
from dpix_io.graph.data_modifiers import SumModifier, AverageModifier, BaseModifier
from dpix_io.graph.utils import get_date_range_from_request, default_date_format
from dpix_io.dashboard.forms import UserLoginDasboardForm
...
class BaseView(View):
    ...
    def fetch_data(self,
                   ...
                   data_y=None) -> Dict:
        all_endpoint_urls = {
            ApiUrls.dpay: settings.REQUESTS_URL.get(name_url, '{url}').format(url=settings.DPAY_V1),
            ApiUrls.dweb: settings.REQUESTS_URL.get(name_url, '{url}').format(url=settings.DWEB_V1)
        }
...
class PostsFeeWeekly(BaseView):
    ...
    GET param:
        date_to = default date (yesterday)
        date_from = default 7 days ago
        currency =  (BBD, bex, usd) defauld BBD
    """
...
class CommentsPercentage(BaseView):
    ...
    created through dPix relatively to all comments
    """

    title = 'Comments percent'
    subtitle = 'Comments through dPix / all comments'
...
class GetHotTopNewCount(BaseView):
    ...
    headers = [
        {'Date': 'string'},
        {'dPay hot': 'number'}, {'dPay hot': 'number'},
        {'dPay top': 'number'}, {'dPay top': 'number'},
        {'dPay new': 'number'}, {'dPay new': 'number'}
    ]
...
class DeviceUsage(BaseView):
    ...
    def get_data(self):
        api_url = settings.REQUESTS_URL.get(self.name_url, '{url}').format(url=settings.DPAY_V1)
...
class GetAllGraphsOnePage(BaseView):
    ...
    names_stats_endpoints = {
        ...
        'total_active_power_daily': {'title': 'Total Active Power', 'data_x': 'created_at', 'data_y': 'total_dpay_power'},
    }

    def get_data(self, name_graph):
        api_url = settings.REQUESTS_URL.get(name_graph, '{url}').format(url=settings.DPAY_V1)
        res = {
            'data': [],
